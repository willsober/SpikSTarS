...
def sep_user_device_gt():
    ...
    dataDir = 'heterogeneity+activity+recognition/Activity recognition exp/'
    fileInList = os.listdir(dataDir)

    saveDir = 'heterogeneity+activity+recognition/avtivity_data_separated/'
...
def combine_acc_gyr():
    dataDir = 'heterogeneity+activity+recognition/avtivity_data_separated/'
    saveDir = 'heterogeneity+activity+recognition/avtivity_data_acc_gyr/'
    if not os.path.exists(saveDir):
        os.mkdir(saveDir)
    user_list = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i']
            ...
            for gt in gt_list:
                ...
                while idx1 < df_acc.shape[0] and idx2 < df_gyr.shape[0]:
                    ...
                    if abs(curTime1 - curTime2) < 0.1*timeScale:
                        ...
                        df_acc_gyr = pd.concat([df_acc_gyr, pd.DataFrame(rows)], ignore_index=True)
                        idx1 += 1
                        idx2 += 1
